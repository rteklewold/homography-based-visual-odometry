...
class VisualOdometry:
    ...
    def compute_relative_transform(self, matches: List[cv2.DMatch], update_src_frame: bool) -> \
        ...
        mat_H, mask = cv2.findHomography(src_pts, incoming_pts, cv2.RANSAC, 5.0)
        ...
        #src_pts_normalized = np.zeros(src_pts_homo.shape)
        src_pts_normalized = np.matmul(np.linalg.inv(self.camera_intrinsic), src_pts_homo.transpose())
        print(src_pts_normalized.shape)
        ...
        if num_candidates > 1:
            ...
            if len(rots) > 1:
                ...
                angle_0 = np.arccos(normals[0].transpose()@self.plane_normal_src)
                angle_1 = np.arccos(normals[1].transpose()@self.plane_normal_src)
        ...
        if rots:
            ...
            if self.plane_d_src < 0:
                ...
                self.plane_d_src = normals[0].transpose()@self.plane_origin_src
            ...
            incoming_M_src[0:3, 0:3] = rots[0]
            #incoming_M_src[0:3, 3] = trans[0].reshape((3, 1))
            incoming_M_src[0:3, 3] = trans[0].flatten()
            print(incoming_M_src)
            #incoming_M_src = np.concatenate((rots[0], trans[0]), axis = 1) # TODO create the transfromation matrix from src camera frame to
            #incoming_M_src = np.concatenate((incoming_M_src, np.array([0, 0, 0, 1])), axis =0)
            ...
            if update_src_frame:
                ...
                src_pts_3d = depth*src_pts_normalized.transpose() # TODO: compute 3d coordinate in src camera frame for every inlier,
                # TODO: (continue) result should be a matrix of shape <num_inliers, 3>
                print('src_pts_3d shape', )

                # map these points to incoming frame
                #incoming_pts = incoming_M_src @ src_pts_3d.transpose()  # TODO: result should have shape <num_inliers, 3>
                incoming_pts = rots[0]@src_pts_3d.transpose() + trans[0]
                # compute new d by averaging projection of every point onto plane's normal vector
                #self.plane_d_src = 0.00001 # TODO
                self.plane_d_src = np.mean(incoming_pts.transpose()@self.plane_normal_src)
